...
void adjacencylist_print(AdjacencyList *adjacencylist, AdjacencyList adjlists[],
		FILE *file) {
	EdgeNode *temp1 = adjacencylist->head;
	fprintf(file, "Neighbors:\n");
	int count = 1;
	while (temp1 != NULL) {
		Vertex adjacent = adjlists[temp1->edge.destination].vertex;
		Config temp = adjacent.configuration;
		int j;
		fprintf(file, "\t%d) Vertex %d:\n", count++, temp1->edge.destination);
		for (j = 0; j < 3; j++) {
			fprintf(file, "\t\t%d %d %d\n", temp.board[j][0], temp.board[j][1],
					temp.board[j][2]);
		}
		temp1 = temp1->next;
	}
